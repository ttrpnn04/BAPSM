...
BEGIN
    ...
    UPDATE dbo.StockTransactions SET DocumentID = @DocumentID WHERE TransactionID = @TransactionID;
    FETCH NEXT FROM orphan_cursor INTO @TransactionID, @TransactionTypeID, @TxnDate, @RefNo, @Note, @CreatedAt, @CreatedBy;
END
CLOSE orphan_cursor;
...
IF EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'dbo.StockTransactions') AND name = N'DocumentID' AND is_nullable = 1
)
AND NOT EXISTS (SELECT 1 FROM dbo.StockTransactions WHERE DocumentID IS NULL)
BEGIN
    IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_StockTransactions_DocumentID' AND object_id = OBJECT_ID(N'dbo.StockTransactions'))
    ...
    ALTER TABLE dbo.StockTransactions WITH CHECK
        ADD CONSTRAINT FK_StockTransactions_Documents FOREIGN KEY (DocumentID) REFERENCES dbo.StockDocuments (DocumentID);

    CREATE INDEX IX_StockTransactions_DocumentID ON dbo.StockTransactions (DocumentID);
END
GO

SELECT COUNT(*) AS Documents FROM StockDocuments;
SELECT COUNT(*) AS Txns FROM StockTransactions WHERE DocumentID IS NOT NULL;
GO
